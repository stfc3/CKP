 ...
 * @author dmin
 */
public class ReportDistribute extends GenericForwardComposer {

    private static final Logger logger = Logger.getLogger(ReportDistribute.class);
    @WireVariable
    ...
    private String reportName = "reportDistribute.rptdesign";
    private String fileName = "BaoCaoThueCanPhanPhoi";
...
//        List<Construction> constructionSelectDefault= new ArrayList<>();
    ...
    private boolean validateParam() {
        if (!StringUtils.isValidString(cbxCustomer.getValue()) || Labels.getLabel("option").equals(cbxCustomer.getValue())) {
            Messagebox.show("Hãy chọn khách hàng", "Lỗi", Messagebox.OK, Messagebox.ERROR);
            return false;
        }
        if (!StringUtils.isValidString(cbxConstruction.getValue()) || Labels.getLabel("option").equals(cbxConstruction.getValue())) {
            Messagebox.show("Hãy chọn công trình", "Lỗi", Messagebox.OK, Messagebox.ERROR);
            return false;
        }
