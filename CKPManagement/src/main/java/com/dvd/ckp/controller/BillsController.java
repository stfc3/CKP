...
//    private List<Bills> lstBillsFilter;
    ...
    public void doAfterCompose(Component comp) throws Exception {
        ...
        // SpringUtil.getBean(SpringConstant.CONSTRUCTION_SERVICES);
        // customerService = (CustomerService)
        // SpringUtil.getBean(SpringConstant.CUSTOMER_SERVICES);
        ...
        // SpringUtil.getBean(SpringConstant.LOCATION_SERVICES);
        ...
        // List<Construction> lstCon = new
        // ArrayList<>(memory.getConstructionCache().values());
        ...
        mapCustomer = lstCustomer.stream()
                .collect(Collectors.toMap(Customer::getCustomerId, vlstCustomer -> vlstCustomer));

        modelListConstruction = new MyListModel(lstConstructions);
        cbFilterConstruction.setModel(modelListConstruction);

        mapConstruction = lstConstructions.stream()
                .collect(Collectors.toMap(Construction::getConstructionId, vlstConstructions -> vlstConstructions));
    ...
    private void getDataInRow(List<Component> lstCell, Bills bills) {
        ...
        if (component != null && component instanceof Datebox) {
            try {
                ...
                bills.setPrdID(DateTimeUtils.convertDateToString(dtPrdID.getValue(), Constants.FORMAT_DATE));
                bills.setStrDateInput(
                        DateTimeUtils.convertDateToString(dtPrdID.getValue(), Constants.FORMAT_DATE_DD_MM_YYY));
            } catch (Exception e) {
